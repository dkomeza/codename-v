      ...
      <div className="flex flex-col gap-4 justify-start">
        {favoriteEvents.map((event) => {
          ...
          return (
            <div
              key={event.id}
              className={`relative w-64 h-64 rounded-lg overflow-hidden shadow-lg select-none transition-opacity duration-500 ${isRemoving ? "opacity-50 pointer-events-none" : "opacity-100"
                }`}
            >
              <button
                onClick={(e) => {
              ...
              >
                <Heart
                  className={`w-6 h-6 ${event.favorite
